package xyz.reknown.spigetaddons.features;
...
public class RenameBridge {
    private final Pattern chatPattern = Pattern.compile("^(§2Guild|§3Officer) > (?:\\S+ )?(\\w{3,16})(?: §[a-z]\\[[A-Z]+])?§f: .+");

    @SubscribeEvent
    public void reformatMessage(ClientChatReceivedEvent event) {
